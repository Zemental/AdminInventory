...
class ChipModel {
    ...
    function mostrarChips() {
        ...
        while($row = mysqli_fetch_row($this->result)){            
            ...
            if ($row[8] == 1){
                echo '<tr>
                    <td style="text-align:center; font-size: 12px; height: 10px; width: 3%;text-align:center; font-weight:bold; background:#CCFF99;">'.$item.'</td>';
                    ...
                    if ($row[7] == 'M') {
                        ...
                        <div class="hidden-sm hidden-xs action-buttons">                                
                            <a href="#" style="margin-right:20px;" title="Ver datos">
                                <span class="green">
                                    <i class="ace-icon fa fa-search bigger-120" onclick="verChip('.$row[0].');"></i>
                                </span>
                            </a>';
                    }
    ...
    function mostrador() {
        $this->prepararConsultaGestionarChips('opc_mostrador_chip');
        $this->cerrarAbrir();
        echo 1;
    }    
