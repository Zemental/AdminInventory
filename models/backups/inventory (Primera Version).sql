...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 22-06-2016 a las 00:57:15
...
BEGIN
  IF opcion = 'opc_mostrar_accesorios' THEN
    ...
    IF opcion = 'opc_datos_accesorio' THEN
      SELECT P.idProducto, A.idTipoAccesorio, A.codigo, A.descripcion, A.cantidad
          FROM Productos P
            INNER JOIN Accesorios A ON P.idProducto = A.idProducto
            INNER JOIN TipoAccesorio TA ON A.idTipoAccesorio =            TA.idTipoAccesorio
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_control_protector`(IN `opcion` VARCHAR(65), IN `codigo` INT, IN `tipo` INT, IN `modeloCelular` VARCHAR(100), IN `cantidad` INT, IN `estado` INT)
    NO SQL
BEGIN
  IF opcion = 'opc_mostrar_protectores' THEN
      SELECT PR.idProducto, PR.idProtector, TP.nombre, PR.modeloCelular, PR.cantidad, S.nombre, P.estado, P.activo
          FROM Productos P 
    ...
    IF opcion = 'opc_registrar_protector' THEN      
        ...
        INSERT INTO Productos (idTipoProducto, fechaIngreso, activo)        VALUES (3, now(), 1);
          ...
          INSERT INTO Protectores (idProducto, idTipoProtector,       modeloCelular, cantidad) VALUES(@PRODUCTO, tipo, modeloCelular, cantidad);       
        
    END IF;
    
    IF opcion = 'opc_datos_protector' THEN
      SELECT P.idProducto, PR.idTipoProtector, PR.modeloCelular, PR.cantidad
          FROM Productos P
            INNER JOIN Protectores PR ON P.idProducto = PR.idProducto
            INNER JOIN TipoProtector TP ON PR.idTipoProtector = TP.idTipoProtector
            WHERE PR.idProducto = codigo;
    END IF;
    
    IF opcion = 'opc_editar_protector' THEN
      UPDATE Protectores PR
          SET PR.idTipoProtector = tipo, PR.modeloCelular = modeloCelular, PR.cantidad = cantidad
          WHERE PR.idProducto = codigo;
    END IF;
...
(1, 4, 3, 'BAT001', 'BATERIA PARA SAMSUMG', 24);
...
(1, 1, 'IMEI001', 'SER001', 'SAMSUMG', 'SAMSUMG GALAXY S7');
...
(1, 2, 'ICC001', '51938254410', 'MOVISTAR');
...
INSERT INTO `detallemovimiento` (`numDetalle`, `numMovimiento`, `cantidad`, `idProducto`) VALUES
(1, 1, 1, 1),
(2, 1, 1, 2),
(3, 2, 1, 3),
(4, 2, 1, 4);
...
(1, 1, '2016-06-21', 2, b'1'),
(2, 2, '2016-06-21', 2, b'1');
...
(3, 3, 2, 'A', '2016-06-21', b'1'),
(4, 4, 2, 'A', '2016-06-21', b'1');
...
('BOLETA-001', 1, 'B', '2016-06-21', 1225, b'1');
